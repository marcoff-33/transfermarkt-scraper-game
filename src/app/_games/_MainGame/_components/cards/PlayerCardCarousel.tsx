      ...
      <button className={`overflow-hidden grow shadow-[5px_5px_15px_rgba(0,0,0,0.250)] duration-1000 transition-all p-1 h-full bg-background-deep md:w-full w-full ${playerData.scrapedPlayerData.marketValueNumber == 0 ? "shadow-text-primary" : getPlayerColor(playerData.scrapedPlayerData.marketValueNumber, "shadow")} `} onClick={handleButtonClick} disabled={loadingImg}>
        <div className="min-h-full flex flex-col justify-center items-center relative overflow-hidden">
          <div className="bg-background-deep overflow-hidden flex items-center justify-center flex-col min-h-full min-w-full relative grow ">
            {playerData.playerName !== "" && <Image src={playerData.scrapedPlayerData.playerHeroImg} alt={"player card"} fill sizes="400px" className={`self-center object-cover object-top rounded-lg transition-all duration-1000 w-full h-full grow sm:block hidden ${loadingImg ? "blur-xl animate-pulse backdrop-blur-lg" : "blur-none"} ${playerData.playerName == "" ? "hidden" : "block"}`} />}
            {playerData.playerName !== "" && (
              <Image
                ...
                fill
                sizes="200px"
                className={`z-[50] self-center rounded-lg  object-scale-down transition-all duration-1000 w-full h-full sm:hidden ${loadingImg ? "blur-xl animate-pulse backdrop-blur-lg" : "blur-none"}
          ...
          `}
            ...
            <div className={`z-50 left-1/2 transform -translate-x-1/2 sm:rounded-full text-nowrap absolute md:bottom-7 bottom-0 sm:top-auto self-center text-center w-fit transition-colors md:duration-1000 shadow-md md:backdrop-blur-lg px-2 shadow-black bg-background-mid/50 ${loadingImg ? "invisible text-zinc-500" : "block text-white"}`}>
              <p className={`hidden md:block whitespace-nowrap sm:animate-none z-50 px-5 ${loadingImg ? "text-red-500" : "text-text-primary"}`}>{playerData.scrapedPlayerData.fullPlayerName}</p>
              <p className="md:hidden animate-marqueeSlow whitespace-nowrap sm:animate-none z-50 text-text-primary px-5">{playerData.scrapedPlayerData.shortPlayerName}</p>
            </div>
          </div>
        </div>
      </button>
